...
{
    static final boolean USE_RANGE_SENSOR = false;
    static final boolean USE_LINE_DETECTOR = false;
    static final boolean USE_ODS_LINE_DETECTOR = false;
    static final boolean USE_COLOR_SENSOR = false;
    ...
    public Robot(TrcRobot.RunMode runMode)
    {
        ...
        if (USE_ANALOG_GYRO)
        {
            gyro = new FtcAndroidGyro("analogGyro");
            ((FtcAndroidGyro)gyro).calibrate();
            gyro.setYScale(RobotInfo.ANALOG_GYRO_SCALE);
...
//                lineDetectionSensor.setDeviceEnabled(false);
        ...
        //
        leftFrontWheel = new FtcDcMotor("left");
        rightFrontWheel = new FtcDcMotor("right");
        leftRearWheel = new FtcDcMotor("backLeft");
        ...
        encoderXPidCtrl = new TrcPidController(
                ...
                this);
        //encoderXPidCtrl.setOutputRange(-0.75, 0.75);
        ...
        gyroPidCtrl = new TrcPidController(
                ...
                this);
        gyroPidCtrl.setAbsoluteSetPoint(false);
        //gyroPidCtrl.setInverted(true);
...
//        rightButtonPusher.setPosition(RobotInfo.BUTTON_PUSHER_RETRACT_POSITION);
        ...
        ballPickUp = new FtcDcMotor("ballGrabber");
        ballPickUp.setInverted(true);

        conveyor = new FtcDcMotor("elevator");
    ...
    double selectParameter(boolean startNear, AltRyanOpFtcAuto.Alliance alliance, double nearRed, double nearBlue,
                           double farRed, double farBlue)
    {
        if (startNear)
        {
            if (alliance == AltRyanOpFtcAuto.Alliance.RED_ALLIANCE)
            {
                return nearRed;
            }
            else
